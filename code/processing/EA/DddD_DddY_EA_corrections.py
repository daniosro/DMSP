...
# %% 
# Load data
df_data =  pd.read_csv (f'{homedir}/data/raw/EA/20190903_EA.csv')
...
df_blanks = pd.read_csv(f'{homedir}/data/raw/EA/20190903_blanks.csv')
...
df_lin =  pd.read_csv(f'{homedir}/data/raw/EA/20190903_linearity.csv')
df_lin.head()
...
for row in df_data['Area_All']:
    if row <= 19:
        Std_group.append ('very_low')
    elif 19.1 <= row <= 35:
        Std_group.append ('low')
    elif 35.1 <= row <= 60:
        Std_group.append ('medium')
    elif 60.1 <= row <= 85:
        Std_group.append ('high')
    elif row >= 85.1:
        Std_group.append ('very_high')        
...
df_standards.drop(df_standards.index[21], inplace=True)
...
# %%
# Export data table
df_samples.to_csv(f'{homedir}/data/processed/EA/20190903_EA.csv')
