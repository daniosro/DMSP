...
# Filter by DddQ 
df_dddq= df_master[(df_master.Name == 'DddQ')]
df_dddq.head()
# %%
#Conversion of delta values to isotopic ratios
df_dddq['r34_approx'] = ((df_dddq['d34S_approx']/1000)+1)*0.0450045
df_dddq.head()
# %%
#Conversion of isotopic ratios to fractional abundances
df_dddq['f34_approx'] = df_dddq['r34_approx']/(1+df_dddq['r34_approx'])
df_dddq.head()
# %%
...
for i in df_dddq_t_0.Real_conc:
...
df_dddq_t_0['conc_s_cell_lysate']=conc_s_cell_lysate
df_dddq_t_0
# %%
#Create dataframe just for conc of S in cell lysate in each replicate
df_s_cell_lysate = df_dddq_t_0[['Replicate', 'conc_s_cell_lysate']]
df_s_cell_lysate
# %%
# Append the concentration of S in cell lysate to the main enzyme dataframe
# for each replicate                
df_dddq = pd.merge(df_dddq, df_s_cell_lysate,  how='inner', on=['Replicate'], suffixes=('', '_y'))
df_dddq.head()
# %%
#Calculation of 34_F_DMSP
# Append column to the enzyme dataframe with the app. value of ^34F_DMSP
df_dddq['F34_approx_DMSP']= ((df_dddq['conc_s_cell_lysate'] * (df_dddq['f34_approx']-f34_cell_lysate))+ 
                             (df_dddq['Real_conc']*df_dddq['f34_approx']))/df_dddq['Real_conc']
df_dddq.head()
# %%
#Convert fractional abundances to isotopic ratios
df_dddq['R34_approx_DMSP'] = df_dddq['F34_approx_DMSP']/(1-df_dddq['F34_approx_DMSP'])
#Convert isotopic ratios to delta values
df_dddq['d34S_approx_DMSP'] = ((df_dddq['R34_approx_DMSP']/0.0450045)-1)*1000
df_dddq.head()
# %%
# Export master table with the data with all corrections
df_dddq.to_csv(f'{homedir}/data/processed/enzymes/dddq_master.csv')