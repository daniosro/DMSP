...
# Filter by DddD
df_dddd= df_master[(df_master.Name == 'DddD')]
df_dddd.head()
# %%
#Conversion of delta values to isotopic ratios
df_dddd['r34_approx'] = ((df_dddd['d34S_approx']/1000)+1)*0.0450045
df_dddd.head()
# %%
#Conversion of isotopic ratios to fractional abundances
df_dddd['f34_approx'] = df_dddd['r34_approx']/(1+df_dddd['r34_approx'])
df_dddd.head()
# %%
...
for i in df_dddd_t_0.Real_conc:
...
df_dddd_t_0['conc_s_cell_lysate']=conc_s_cell_lysate
df_dddd_t_0
# %%
#Create dataframe just for conc of S in cell lysate in each replicate
df_s_cell_lysate = df_dddd_t_0[['Replicate', 'conc_s_cell_lysate']]
df_s_cell_lysate
# %%
# Append the concentration of S in cell lysate to the main enzyme dataframe
# for each replicate                
df_dddd = pd.merge(df_dddd, df_s_cell_lysate,  how='inner', on=['Replicate'], suffixes=('', '_y'))
df_dddd.head()
# %%
#Calculation of 34_F_DMSP
# Append column to the enzyme dataframe with the app. value of ^34F_DMSP
df_dddd['F34_approx_DMSP']= ((df_dddd['conc_s_cell_lysate'] * (df_dddd['f34_approx']-f34_cell_lysate))+ 
                             (df_dddd['Real_conc']*df_dddd['f34_approx']))/df_dddd['Real_conc']
df_dddd.head()
# %%
#Convert fractional abundances to isotopic ratios
df_dddd['R34_approx_DMSP'] = df_dddd['F34_approx_DMSP']/(1-df_dddd['F34_approx_DMSP'])
#Convert isotopic ratios to delta values
df_dddd['d34S_approx_DMSP'] = ((df_dddd['R34_approx_DMSP']/0.0450045)-1)*1000
df_dddd.head()
# %%
# Shift corrections
#Correct shift in replicate a
#Filter by replicate a
df_dddd_a= df_dddd[(df_dddd.Replicate == 'a')]
#Drop replicate b in the dddd dataframe
df_dddd = (df_dddd[df_dddd['Replicate'] != 'a'])
#Add 0.8 to d34_S_DSMP only in replicate a
df_dddd_a['d34S_approx_DMSP']= df_dddd_a['d34S_approx_DMSP'] + 0.8
#Add corrected replicate a to the dddk dataframe
df_dddd = pd.merge(df_dddd, df_dddd_a, how='outer')
df_dddd.head()
# %%
#Correct shift in replicate c
#Filter by replicate c
df_dddd_c= df_dddd[(df_dddd.Replicate == 'c')]
#Drop replicate b in the dddd dataframe
df_dddd = (df_dddd[df_dddd['Replicate'] != 'c'])
#Subtract 0.9 to d34_S_DSMP only in replicate a
df_dddd_c['d34S_approx_DMSP']= df_dddd_c['d34S_approx_DMSP'] -0.9
#Add corrected replicate a to the dddk dataframe
df_dddd = pd.merge(df_dddd, df_dddd_c, how='outer')
df_dddd.head()
# %%
# Export master table with the data with all corrections
df_dddd.to_csv(f'{homedir}/data/processed/enzymes/dddd_master.csv')
# %%
