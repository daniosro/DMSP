...
# Filter by DmdA 
df_dmda= df_master[(df_master.Name == 'DmdA')]
df_dmda.head()
# %%
#Conversion of delta values to isotopic ratios
df_dmda['r34_approx'] = ((df_dmda['d34S_approx']/1000)+1)*0.0450045
df_dmda.head()
# %%
#Conversion of isotopic ratios to fractional abundances
df_dmda['f34_approx'] = df_dmda['r34_approx']/(1+df_dmda['r34_approx'])
df_dmda.head()
# %%
...
for i in df_dmda_t_0.Real_conc:
...
df_dmda_t_0['conc_s_cell_lysate']=conc_s_cell_lysate
df_dmda_t_0
# %%
#Create dataframe just for conc of S in cell lysate in each replicate
df_s_cell_lysate = df_dmda_t_0[['Replicate', 'conc_s_cell_lysate']]
df_s_cell_lysate
# %%
# Append the concentration of S in cell lysate to the main enzyme dataframe
# for each replicate                
df_dmda = pd.merge(df_dmda, df_s_cell_lysate,  how='inner', on=['Replicate'], suffixes=('', '_y'))
df_dmda.head()
# %%
#Calculation of 34_F_DMSP
# Append column to the enzyme dataframe with the app. value of ^34F_DMSP
df_dmda['F34_approx_DMSP']= ((df_dmda['conc_s_cell_lysate'] * (df_dmda['f34_approx']-f34_cell_lysate))+ 
                             (df_dmda['Real_conc']*df_dmda['f34_approx']))/df_dmda['Real_conc']
df_dmda.head()
# %%
#Convert fractional abundances to isotopic ratios
df_dmda['R34_approx_DMSP'] = df_dmda['F34_approx_DMSP']/(1-df_dmda['F34_approx_DMSP'])
#Convert isotopic ratios to delta values
df_dmda['d34S_approx_DMSP'] = ((df_dmda['R34_approx_DMSP']/0.0450045)-1)*1000
df_dmda.head()

# %%
# Export master table with the data with all corrections
df_dmda.to_csv(f'{homedir}/data/processed/enzymes/dmda_master.csv')